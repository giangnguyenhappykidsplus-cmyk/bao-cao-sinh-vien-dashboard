...
import { Users, GraduationCap, UserMinus, ShieldAlert, AlertTriangle, HeartPulse, TrendingUp, Sparkles, Eye, FileText } from 'lucide-react';
...
function KpiCard({ icon, label, tone, children, caption, active, onClick, onViewStudents, onExportReport, exporting }: {
  ...
  onExportReport: () => void; exporting: boolean;
}) {
  const toneRing = {
  ...
  return (
    <button onClick={onClick} className={`group relative flex w-full flex-col rounded-2xl border p-4 text-left shadow-card transition-all animate-slideUp ${active ? 'border-accent bg-ink-800/90 shadow-glow ring-1 ring-accent/40' : 'border-ink-700/70 bg-ink-850/80 hover:border-ink-600/80 hover:bg-ink-800/80'}`}>
      ...
      <div className="mt-2 flex items-center gap-3">
        <span onClick={(e) => { e.stopPropagation(); onViewStudents(); }} className="inline-flex items-center gap-1 text-[10px] font-medium text-blue-400 transition-colors hover:text-blue-300">
          <Eye className="h-3 w-3" /> Danh sách SV
        </span>
        <span onClick={(e) => { e.stopPropagation(); if (!exporting) onExportReport(); }} className={`inline-flex items-center gap-1 text-[10px] font-medium transition-colors ${exporting ? 'text-slate-500 cursor-wait' : 'text-emerald-400 hover:text-emerald-300'}`}>
...
export function KpiCards({ kpi, quyMoDauKy, dangHocHienTai, activeDrill, onDrill, onViewStudents, onExportReport, exporting }: {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
      ...
      <KpiCard icon={<GraduationCap className="h-4 w-4" />} label="Quy mô & Đang học" tone="good"
        ...
        active={activeDrill === 'dang_hoc'} onClick={() => onDrill('dang_hoc')} onViewStudents={() => onViewStudents('dang_hoc')} onExportReport={onExportReport} exporting={exporting}>
      ...
      <KpiCard icon={<UserMinus className="h-4 w-4" />} label="Thôi học" tone="danger"
        ...
        active={activeDrill === 'thoi_hoc'} onClick={() => onDrill('thoi_hoc')} onViewStudents={() => onViewStudents('thoi_hoc')} onExportReport={onExportReport} exporting={exporting}>
        {num(kpi.thoi_hoc, 'SV')}
      ...
      <KpiCard icon={<ShieldAlert className="h-4 w-4" />} label="Bảo lưu" tone="violet"
        caption="Cộng dồn phát sinh từ tháng A đến tháng B"
        active={activeDrill === 'bao_luu'} onClick={() => onDrill('bao_luu')} onViewStudents={() => onViewStudents('bao_luu')} onExportReport={onExportReport} exporting={exporting}>
        {num(kpi.bao_luu, 'SV')}
      ...
      <KpiCard icon={<AlertTriangle className="h-4 w-4" />} label="Nghỉ học dài ngày" tone="warn"
        ...
        active={activeDrill === 'nghi_hoc_dai_ngay'} onClick={() => onDrill('nghi_hoc_dai_ngay')} onViewStudents={() => onViewStudents('nghi_hoc_dai_ngay')} onExportReport={onExportReport} exporting={exporting}>
      ...
      <KpiCard icon={<ShieldAlert className="h-4 w-4" />} label="Tỷ lệ nhóm nguy cơ" tone="risk"
        ...
        active={activeDrill === 'nhom_nguy_co'} onClick={() => onDrill('nhom_nguy_co')} onViewStudents={() => onViewStudents('nhom_nguy_co')} onExportReport={onExportReport} exporting={exporting}>
      ...
      <KpiCard icon={<HeartPulse className="h-4 w-4" />} label="Quay lại học" tone="accent"
        ...
        active={activeDrill === 'quay_lai'} onClick={() => onDrill('quay_lai')} onViewStudents={() => onViewStudents('quay_lai')} onExportReport={onExportReport} exporting={exporting}>
        {num(kpi.quay_lai, 'SV')}
      </KpiCard>
    </div>
  );
